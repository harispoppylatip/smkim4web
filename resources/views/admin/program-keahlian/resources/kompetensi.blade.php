<div id="kompetensi" class="resource-section max-w-4xl mb-8">
    <div class="bg-white rounded-xl border border-[#e2e2e5] overflow-hidden">
        <div class="px-6 py-4 bg-[#f9f9fc] border-b border-[#e2e2e5]">
            <h3 class="text-sm font-semibold text-[#1a1c1e]">Kompetensi Keahlian</h3>
            <p class="text-xs text-[#737780]">Kelola daftar kompetensi untuk program ini.</p>
        </div>
        <div class="p-6">
            <form method="POST" action="{{ route('admin.program-keahlian.kompetensi.store', $program->id) }}"
                class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
                @csrf
                <div class="md:col-span-2">
                    <label class="text-xs font-semibold text-[#43474f] mb-1 block">Nama Kompetensi</label>
                    <input type="text" name="nama" required placeholder="cth: Instalasi Jaringan Komputer"
            ...
            <table class="w-full resource-table border border-[#e2e2e5] rounded-lg overflow-hidden">
                ...
                <tbody class="divide-y divide-[#e2e2e5]">
                    @forelse ($program->kompetensi as $item)
                        <tr class="hover:bg-[#f9f9fc] transition-colors">
                            <td class="text-[#737780]">{{ $loop->iteration }}</td>
                            <td>{{ $item->nama }}</td>
                            <td class="text-right">
                                <form method="POST"
                                    action="{{ route('admin.program-keahlian.kompetensi.destroy', [$program->id, $item->id]) }}"
                                    class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button onclick="return confirm('Hapus kompetensi?')"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center text-[#737780] py-6">Belum ada kompetensi. Tambahkan
                                kompetensi di atas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
